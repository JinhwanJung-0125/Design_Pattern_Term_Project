#Memory�� Register ���� ������ Bridge Pattern�� ����
...
class MemoryAccess(Access): #Concrete Implementor
    ...
    def access(self, addr, value, readOrWrite, size):
        ...
        if selectMEM == 0x004:      #Text ���� Memory
            pM = self.progMEM

        elif selectMEM == 0x100:    #Data ���� Memory
            pM = self.dataMEM

        elif selectMEM == 0x7ff:    #Stack ���� Memory
            pM = self.stackMEM
...
class Register(MemoryRegister): #Concrete Abstraction   (Register)
    def useToAccess(self, addr, value, readOrWrite):                #addr = Register ��ȣ / value = Register�� ������ �ϴ� �� / readOrWrite = 0: Register �б� , 1: Register ���� 
...
class Memory(MemoryRegister):   #Concrete Abstraction   (Memory)
    def useToAccess(self, addr, value, readOrWrite, size):          #addr = Memory �ּ� / value = Memory�� ������ �ϴ� �� / readOrWrite = 0: Memory �б� , 1: Memory ���� / size = Memory�� �ѹ��� Access�� ũ�� 0: 1byte , 1: half word , 2: word
        return self.access.access(addr, value, readOrWrite, size)

    def useToPrint(self, start, end):       #start: Memory Print ���� ���� / end: Memory Print �� ����
...
class InstructionRegister:              #��ɾ� ���� Data Structure
    def __init__(self, num):
        self.I = num  #32-bits ����
        self.RI = self.RFormat(num)    #32-bits R format���� ��ɾ�
        self.II = self.IFormat(num)    #32-bits I format���� ��ɾ�
        self.JI = self.JFormat(num)    #32-bits J format���� ��ɾ�
...
class ALU:
    @staticmethod
    def invert_endian(inVal):   #Endian ��ȯ �Լ�
    ...
    def ALU(inst, PC, MEM: MemoryRegister, Reg: MemoryRegister):
        ...
        if IR.RI.opcode == 0:       # sll rd rt shamt
            if IR.RI.funct == 0:
                ...
                hi = multiResult >> 32  # ���� 32��Ʈ
                lo = multiResult & 0xffffffff  # ���� 32��Ʈ
            ...
            elif IR.RI.opcode == 15:    #lui rt, imm: load upper immediate// ���� 16bit�� imm�� �ְ� �� 16bit�� 0���� �д�.
...
class setPC(Command):           #Program Counter ����
...
class setSP(Command):           #Stack Pointer ����
    def __init__(self, addr, Reg: MemoryRegister):
        self.addr = addr
        self.Reg = Reg
...
class jumpProgram(Command):     #Ư�� ��ġ�� Jump
...
class setRegister(Command): #Ư�� Register �� ����
...
class setMemory(Command):   #Ư�� Memory��ġ�� �� ����
